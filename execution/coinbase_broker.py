...
class CoinbaseBroker(Broker):
    ...
    def _fetch_balance_with_retry(
        ...
        """Fetch Coinbase balance with retry on transient errors."""
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    log.warning(
                        f"Coinbase fetch_balance failed ({e}), retrying in {delay}s..."
                    )
                    time.sleep(delay)
        if last_error is not None:
            raise last_error
        raise RuntimeError("Coinbase fetch_balance failed without exception")
    ...
    def get_account_info(self) -> dict[str, object]:
        ...
        try:
            ...
            log.error(f"Could not fetch Coinbase balance after retries: {e}")
            raise
