...
class RiskManager:
    def __init__(self, initial_capital: float, position_manager=None, broker_name: str = ""):
        ...
        if broker_name and broker_name in per_broker:
            ...
            log.info(f"Applied per-broker risk overrides for '{broker_name}'")
    ...
    def can_trade(self) -> bool:
        ...
        if current_drawdown >= self.config["max_drawdown"]:
            log.warning(
                f"Max drawdown reached: {current_drawdown * 100:.2f}%. Trading halted."
            )
            return False
        return True
    ...
    def validate_order(
        ...
        if proposed_risk > max_allowable_risk:
            return (
                False,
                f"Proposed trade risk {proposed_risk:.2f} > max allowable {max_allowable_risk:.2f}",
            )

        if (
            self.open_risk + proposed_risk
            > self.current_capital * self.config["max_portfolio_heat"]
        ):
            return (
                False,
                f"Insufficient risk budget. Open risk: {self.open_risk:.2f}, Proposed: {proposed_risk:.2f}",
            )
    ...
    def get_net_exposure(self, latest_prices: dict) -> float:
        ...
        log.debug(
            f"Net exposure computed: {net:.2f} (long {long_exp:.2f}, short {short_exp:.2f})"
        )
        return net
    ...
    def update_portfolio(self, pnl_change: float, open_risk_change: float):
        ...
        log.debug(
            f"Portfolio updated. Capital: {self.current_capital:.2f}, Open Risk: {self.open_risk:.2f}"
        )
